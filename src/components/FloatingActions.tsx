  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-1/2 -translate-y-1/2 z-[100] flex flex-col gap-3 pr-4 max-md:pr-2">
        {/* Contact Form Toggle */}
        <button
          onClick={() => { setFormOpen(!formOpen); setWaOpen(false); }}
        ...
        {waNumber && (
          <button
            onClick={() => { setWaOpen(!waOpen); setFormOpen(false); }}
            className={`group w-12 h-12 max-md:w-10 max-md:h-10 rounded-full flex items-center justify-center
              shadow-[0_4px_20px_rgba(37,211,102,0.3)] transition-all duration-300
              ${waOpen
                ? "bg-[#128C7E] text-white"
                : "bg-[#25D366] text-white hover:scale-110 hover:shadow-[0_6px_28px_rgba(37,211,102,0.5)]"
              }`}
            aria-label="WhatsApp"
          >
            {waOpen ? (
      ...
      {waOpen && (
        <div className="fixed right-20 max-md:right-16 top-1/2 -translate-y-1/2 z-[99] animate-scaleIn">
          <div className="bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.15)] border border-line w-[300px] overflow-hidden">
            <div className="bg-[#25D366] text-white px-5 py-4">
              <div className="font-bold text-[0.95rem]">WilckeWeb</div>
              <div className="text-white/80 text-[0.8rem]">Normalerweise innerhalb einer Stunde</div>
            </div>
            <div className="p-5">
              <div className="bg-bg-soft rounded-xl p-3 mb-4">
                <p className="text-[0.85rem] text-text leading-relaxed">
                  Hallo! Schreiben Sie Ihre Nachricht und starten Sie den Chat per WhatsApp.
                </p>
              </div>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3.5 py-2.5 rounded-xl border border-line-strong bg-white text-text text-[0.85rem]
                    focus:outline-none focus:ring-2 focus:ring-[#25D366]/20 focus:border-[#25D366] transition-all duration-300"
                />
                <a
                  href={`https://wa.me/${waNumber}?text=${encodeURIComponent(waMessage || "Hallo, ich habe eine Frage zu meinem Website-Projekt.")}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-11 h-11 shrink-0 rounded-full bg-[#25D366] text-white flex items-center justify-center
                    hover:scale-105 hover:shadow-[0_4px_16px_rgba(37,211,102,0.4)] transition-all duration-300"
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {formSent ? (
                ...
                <form onSubmit={handleSubmit} className="grid gap-3">
                  ...
                  <textarea
                    required
                    rows={3}
                    placeholder="Wie k&ouml;nnen wir helfen?"
      ...
      {(waOpen || formOpen) && (
        <div
          ...
          onClick={() => { setWaOpen(false); setFormOpen(false); }}
        />
      )}
    </>
  );
